...
export function AboutPageContent() {
  ...
  return (
    <main className="project-story-page min-h-screen bg-background">
      <div className="project-story-page__backdrop fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.08),_transparent_28%),radial-gradient(circle_at_bottom_right,_rgba(255,255,255,0.06),_transparent_24%)]" />
      <div className="project-story-page__content relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-10 sm:px-6 lg:px-8">
        <header className="project-story-header flex flex-col gap-6 border-b border-border/50 pb-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="project-story-header__brand flex items-center gap-3">
            <div className="project-story-header__emblem relative">
              <Ghost className="h-9 w-9 text-primary" />
              <div className="project-story-header__emblem-glow absolute inset-0 -z-10 rounded-full bg-primary/20 blur-xl" />
            </div>
            <div className="project-story-header__copy">
              <p className="project-story-header__eyebrow text-sm uppercase tracking-[0.3em] text-muted-foreground">GhostTrace</p>
              <h1 className="project-story-header__title text-3xl font-semibold tracking-tight sm:text-4xl">
                {language === "ru" ? "О проекте" : "About The Project"}
              </h1>
            </div>
          </div>

          <div className="project-story-header__actions flex flex-wrap items-center gap-3 text-sm">
            <Link href="/" className="project-story-header__action inline-flex items-center gap-2 rounded-full border border-border/60 bg-secondary/40 px-4 py-2 text-foreground transition-colors hover:bg-secondary/70">
              <ArrowLeft className="h-4 w-4" />
              {language === "ru" ? "Назад к поиску" : "Back to Search"}
            </Link>
            <Link href="/admin/login" className="project-story-header__action inline-flex items-center gap-2 rounded-full border border-border/60 bg-secondary/40 px-4 py-2 text-foreground transition-colors hover:bg-secondary/70">
        ...
        <section className="project-story-hero grid gap-8 py-10 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="project-story-hero__copy space-y-6">
            <span className="project-story-hero__eyebrow inline-flex items-center gap-2 rounded-full border border-border/60 bg-card/60 px-3 py-1 text-xs uppercase tracking-[0.25em] text-muted-foreground">
              <Radar className="h-3.5 w-3.5" />
              OSINT Username Intelligence
            </span>
            <div className="project-story-hero__text space-y-4">
              <h2 className="project-story-hero__title max-w-3xl text-4xl font-semibold leading-tight tracking-tight sm:text-5xl">
                {language === "ru"
              ...
              <p className="project-story-hero__description max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
                {language === "ru"
          ...
          <div className="project-story-hero__feature-grid grid gap-4">
            {localizedFeatures.map(({ icon: Icon, title, description }) => (
              <article key={title} className="project-feature-card rounded-3xl border border-border/60 bg-card/55 p-5 backdrop-blur-sm transition-colors hover:border-primary/30">
                <div className="project-feature-card__icon mb-3 inline-flex rounded-2xl bg-primary/12 p-2 text-primary">
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="project-feature-card__title text-lg font-medium">{title}</h3>
                <p className="project-feature-card__description mt-2 text-sm leading-6 text-muted-foreground">{description}</p>
              </article>
            ))}
          </div>
        </section>

        <section className="project-story-sections grid gap-6 pb-16">
          <article className="project-story-card rounded-[2rem] border border-border/60 bg-card/60 p-6 sm:p-8">
            <div className="project-story-card__header mb-6 flex items-center justify-between gap-3">
              <div className="project-story-card__copy">
                <p className="project-story-card__eyebrow text-sm uppercase tracking-[0.3em] text-muted-foreground">
                  {language === "ru" ? "Русский" : "English"}
                </p>
                <h2 className="project-story-card__title mt-2 text-2xl font-semibold">
                  {language === "ru" ? "О проекте" : "About the project"}
                </h2>
              </div>
              <span className="project-story-card__badge rounded-full border border-border/60 px-3 py-1 text-xs text-muted-foreground">
                {language.toUpperCase()}
              </span>
            </div>

            <div className="project-story-card__sections space-y-5">
              {localizedSections.map((section) => (
                <div key={section.title} className="project-story-card__section rounded-2xl border border-border/50 bg-background/40 p-5">
                  <h3 className="project-story-card__section-title text-lg font-medium">{section.title}</h3>
                  <p className="project-story-card__section-text mt-2 text-sm leading-7 text-muted-foreground">
                    {section.body}
                  </p>
                </div>
              ))}
